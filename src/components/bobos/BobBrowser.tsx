...
import { ArrowLeft, ArrowRight, RotateCw, Globe, Home, Plus, X, Search } from "lucide-react";
...
const INTERNAL_PAGES: Record<string, { title: string; content: React.ReactNode }> = {
  "bobos://home": {
    ...
    content: (
      <div className="max-w-xl mx-auto text-center space-y-4 pt-20">
        ...
        <p className="text-foreground/70 text-sm">Type a URL or search with Google</p>
...
export function BobBrowser() {
  ...
  const navigate = (url: string) => {
    ...
    if (url.startsWith("bobos://")) {
      ...
      finalUrl = getGoogleSearchUrl(url);
      title = `Search: ${url}`;
      ...
      <div className="flex-1 overflow-hidden">
        {isInternal ? (
          <div className="p-6 overflow-auto h-full">
            {INTERNAL_PAGES[currentTab.url]?.content ?? INTERNAL_PAGES["bobos://home"]!.content}
          </div>
        ) : (
          <iframe
            src={currentTab.url}
